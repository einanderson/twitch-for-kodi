...
def get_private_oauth_token():
    # Prefer the device-login store (kimne78/Turbo, auto-refreshed); fall back to the manual
...
def _write_oauth_store(data):
    ...
    compatibility (mirror is never read back for refresh, so a clobber is harmless). Never
    logs token values."""
    try:
...
#
# Mirrors the main store above but for the kimne78 "private" credentials used by usher/GQL
...
def ensure_valid_private_token(force=False):
    """Refresh the private (kimne78/Turbo) token via its stored refresh_token when (near)
    ...
    with _OAuthLock(_PRIVATE_LOCK_FILE):
        ...
        if access_token and not force and time.time() < expiry:
            return access_token
        ok, data = device_oauth.refresh_access_token(_private_client_id(), refresh_token)
        if ok and data.get('access_token'):
...
def use_gql_search():
    # search_backend: 0 = Website (GQL, fuzzy ranking), 1 = Helix (search/channels)
    try:
